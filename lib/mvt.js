...
class MVTLayer {
  ...
  toBuffer() {
    ...
    // Features (tag 2) - nested messages with tag=3
    const featureBuffers = [];
    for (const feature of this.features) {
      const featureBuffer = this._encodeFeature(feature);
      featureBuffers.push(Buffer.from([3])); // Feature tag
      featureBuffers.push(this._encodeVarint(featureBuffer.length));
      featureBuffers.push(featureBuffer);
    }
    const featuresData = Buffer.concat(featureBuffers);
    buffers.push(Buffer.from([2]));
    buffers.push(this._encodeVarint(featuresData.length));
    buffers.push(featuresData);
    
    // Keys (tag 3)
    const keyBuffers = [];
    keyBuffers.push(this._encodeVarint(this.keys.length));
    for (const key of this.keys) {
      const keyBuffer = Buffer.from(key);
      keyBuffers.push(this._encodeVarint(keyBuffer.length));
      keyBuffers.push(keyBuffer);
    }
    const keysData = Buffer.concat(keyBuffers);
    buffers.push(Buffer.from([3]));
    buffers.push(this._encodeVarint(keysData.length));
    buffers.push(keysData);
    
    // Values (tag 4)
    const valueBuffers = [];
    valueBuffers.push(this._encodeVarint(this.values.length));
    for (const value of this.values) {
      const valueBuffer = this._encodeValueBuffer(value);
      valueBuffers.push(this._encodeVarint(valueBuffer.length));
      valueBuffers.push(valueBuffer);
    }
    const valuesData = Buffer.concat(valueBuffers);
    buffers.push(Buffer.from([4]));
    buffers.push(this._encodeVarint(valuesData.length));
    buffers.push(valuesData);
...
class MVTTile {
      ...
      switch (tag) {
        ...
        case 2: // Features
          ...
          while (offset < featureEnd) {
            const { value: featTag, bytesRead: featTagBytes } = MVTTile._decodeVarint(buffer, offset);
            offset += featTagBytes;
            const { value: featLen, bytesRead: featLenBytes } = MVTTile._decodeVarint(buffer, offset);
            offset += featLenBytes;
            
            // Feature starts with tag=3
            if (featTag === 3) {
              const feature = MVTTile._parseFeature(buffer, offset, offset + featLen);
              if (feature) {
          ...
          break;
        case 3: // Keys
          const keysEnd = offset + length;
          const { value: numKeys } = MVTTile._decodeVarint(buffer, offset);
          const numKeysBytes = MVTTile._varintLength(numKeys);
          offset += numKeysBytes;
          for (let i = 0; i < numKeys; i++) {
            const { value: keyLen, bytesRead: keyLenBytes } = MVTTile._decodeVarint(buffer, offset);
            offset += keyLenBytes;
            keys.push(buffer.toString('utf8', offset, offset + keyLen));
            offset += keyLen;
          }
          break;
        case 4: // Values
          const valuesEnd = offset + length;
          const { value: numValues } = MVTTile._decodeVarint(buffer, offset);
          const numValuesBytes = MVTTile._varintLength(numValues);
          offset += numValuesBytes;
          for (let i = 0; i < numValues; i++) {
            const { value: valLen, bytesRead: valLenBytes } = MVTTile._decodeVarint(buffer, offset);
            offset += valLenBytes;
            const value = MVTTile._parseValue(buffer, offset, valLen);
            values.push(value);
            offset += valLen;
          }
          break;
  ...
  static _parseFeature(buffer, offset, endOffset) {
    let id = 0;
    let type = 0;
    let geometry = [];
    let properties = [];
      ...
      switch (tag) {
        ...
        case 4: // Geometry
          ...
          while (currentOffset < geomEnd) {
            const { value: cmd, bytesRead: cmdBytes } = MVTTile._decodeVarint(buffer, currentOffset);
            geometry.push(cmd);
        ...
        case 5: // Properties
          ...
          while (currentOffset < propEnd) {
            const { value: prop, bytesRead: propBytes } = MVTTile._decodeVarint(buffer, currentOffset);
